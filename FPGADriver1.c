...
#define S_FUNCTION_NAME  FPGADriver1
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    //Compensate real-time communication start delay
    bufi = (int)ssGetIWorkValue(S,4);
    if (bufi) {
        err = recv(*sk1,Rx_Buf,118,0);
        buf_old=Rx_Buf[1];
        while (Rx_Buf[1]==buf_old) {
            memset(Tx_Buf,0,20);
            Tx_Buf[0]=0;
            Tx_Buf[1]=Rx_Buf[1];
            Tx_Buf[2]=Rx_Buf[2];
    ...
    bufi = (int)ssGetIWorkValue(S,5)+1;
    if (bufi==256) bufi=0;
    ...
    if (Rx_Buf[1]==bufi) {
        ssSetIWorkValue(S,5,Rx_Buf[1]);
        memcpy(y,Rx_Buf,4);
        fix1=(Rx_Buf[5]<<8)|Rx_Buf[4]; sig_out[0]=(double)(fix1);
        fix1=(Rx_Buf[7]<<8)|Rx_Buf[6]; sig_out[1]=(double)(fix1);
        fix1=(Rx_Buf[9]<<8)|Rx_Buf[8]; sig_out[2]=(double)(fix1);
        fix1=(Rx_Buf[11]<<8)|Rx_Buf[10]; sig_out[3]=(double)(fix1);
